<script lang="ts">
	...
	$: if ($mobile) {
		slideInAnimation = $knowledgeManager ? '-translate-x-0' : '-translate-x-full';
	} else {
		slideInAnimation = $knowledgeManager ? 'translate-y-0' : 'translate-y-[90dvh]';
	}
...
<Dialog
	...
	class="p-0 md:min-h-[300px] h-screen w-screen md:max-h-[436px] md:min-w-[750px] md:max-w-[800px] transition-transform duration-[{animationDuration}ms] ease-out {slideInAnimation}"
>
	<DialogHeader
		slot="header"
