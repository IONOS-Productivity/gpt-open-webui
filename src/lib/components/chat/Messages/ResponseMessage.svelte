...
{#key message.id}
	<div
		class="flex flex-wrap w-full message-{message.id} text-sm"
		id="message-{message.id}"
		dir={$settings.chatDirection}
	>
		<div class={`order-1 sm:shrink-0 ${($settings?.chatDirection ?? 'LTR') === 'LTR' ? 'mr-3' : 'ml-3'}`}>
			<ModelProfile model={model} />
		</div>

		<div class="md:flex-1 order-3 md:order-2 w-full md:w-auto md:pl-1">
			<div>
		...
		{#if !edit}
			<div
				...
				class="ml-0 md:ml-[44px] order-2 md:order-3 flex justify-end md:justify-start grow md:grow-0 md:basis-full overflow-x-auto buttons text-blue-800 dark:text-gray-500 mt-0.5"
			>
